...
write_xlsx(obj.markers,paste(base,new,sample,"/H_markers.xlsx",sep=""))
gc()

new.cluster.ids <- c("Mesenchymal", "Urothelial", "Mesenchymal", "Endothelial", "EMT", "Urothelial", "Mesenchymal", "SMC", "Urothelial", "Urothelial", "VSMC", "Immune", "Neurons")
...
obj <- ScaleData(obj, features = all.genes)
obj.markers <- FindAllMarkers(obj, only.pos = TRUE, min.pct = 0.05, logfc.threshold = 0.1)
write_xlsx(obj.markers,paste(base,new,sample,"/H_annotated_markers.xlsx",sep=""))
obj_H@meta.data$SampleID <- c(rep("H", times = dim(obj_H)[2]))

saveRDS(obj_H,paste(base,new,sample,"/H_SeuratObject.rds",sep=""))
...
# new <- "/Step 2 - Analysis (Manuscript)/01 - Clustering"
# sample <- "/H"
# obj <- readRDS(paste(base,new,sample,"/H_SeuratObject.rds",sep=""))
# plotUMAP <- DimPlot(obj, reduction = "umap",label = TRUE, repel = TRUE, pt.size=0.5)
# plotUMAP
# obj.markers <- read_excel(paste(base,new,sample,"/H_annotated_markers.xlsx",sep=""))
# View(obj.markers)